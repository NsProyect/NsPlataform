class SitiosController < ApplicationController
  ...
  # GET /sitios.json
  def index
    @sitios = Sitio.search(params[:search], params[:page])
  end
  ...
  def destroy
    @sitio.destroy
    respond_to do |format|
      format.html { redirect_to sitios_url, notice: 'Sitio Eliminado.' }
  ...
  private
    ...
    def sitio_params
      params.require(:sitio).permit(:nombre, :descripcion,:direccion, :image)
    end
end
